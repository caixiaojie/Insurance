...
class StatusBarCompatLollipop {
     ...
     * @param color
     */
    static void setStatusBarColor(Activity activity,ViewGroup viewGroup,int color) {
        Window window = activity.getWindow();
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        if (color!=0) {
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        ...
        if(viewGroup!=null){
            ...
            if (viewGroup.getTag() == null) {
                ...
                viewGroup.setPadding(viewGroup.getPaddingLeft(),viewGroup.getPaddingTop()+getStatusBarHeight(activity),viewGroup.getPaddingRight(),viewGroup.getPaddingBottom());
